...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  const quantityTodosCreatedByUser = user.todos.length;

  if (userPlan) {
    return next();
  }

  if (!userPlan && quantityTodosCreatedByUser < 10) {
    return next();
  } else {
    return response
      .status(403)
      .json({ error: "Free plan can create only 10 todos" });
  }
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!isValidId) {
    return response.status(400).json({ error: "Indalid ID!" });
  }

  const findTodoByUserId = findUserByUsername.todos.find(todo => todo.id === id)
